...
|  参数   | 默认值 | 描述                                                 |
| :-----: | :----: | :--------------------------------------------------- |
| heading |   0    | 偏航角，正北为 0，顺时针方向逐渐增加，范围[0, 360]   |
|  pitch  |  -90   | 俯仰角，俯视为-90，向上为正，向下为负，范围[-90, 90] |
|  roll   |   0    | 翻滚角                                               |
...
const center = Cesium.Cartesian3.fromDegreesArrayHeights(102.7357, 38.025, 200);
const radius = 500;
...
如果需要页面一开始的视角就固定在某个城市或园区时，就可以使用默认相机视角。
...
document.addEventListener("keyup", (e) => {
  const format = (num: number, decimals: number = 6) => Number(num.toFixed(decimals));
  ...
  switch (e.code) {
    case "KeyC":
      if (viewer.camera) {
        ...
        const lng = format(Cesium.Math.toDegrees(cartographic.longitude));
        const lat = format(Cesium.Math.toDegrees(cartographic.latitude));
        const height = format(cartographic.height, 2);
        const heading = format(Cesium.Math.toDegrees(camera.heading), 0);
        const pitch = format(Cesium.Math.toDegrees(camera.pitch), 0);
        const roll = format(Cesium.Math.toDegrees(camera.roll), 0);

        const cameraObj = { lng, lat, height, heading, pitch, roll };
        const cameraArr = [lng, lat, height, heading, pitch, roll];
        console.log("相机位置 ==> ", cameraObj, cameraArr);
      }
      break;
    case "KeyD":
      if (viewer.camera) {
        // 计算当前视角的近似可见矩形框
        const currentExtent = viewer.camera.computeViewRectangle(viewer.scene.globe.ellipsoid, new Cesium.Rectangle());
        if (currentExtent) {
          const west = format(Cesium.Math.toDegrees(currentExtent.west));
          const south = format(Cesium.Math.toDegrees(currentExtent.south));
          const east = format(Cesium.Math.toDegrees(currentExtent.east));
          const north = format(Cesium.Math.toDegrees(currentExtent.north));

          const extentObj = { west, south, east, north };
          const extentArr = [west, south, east, north];
          console.log("视角范围 ==> ", extentObj, extentArr);
        }
      }
      break;
  }
