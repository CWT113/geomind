...
> [!WARNING] 注意
>
> `viewer.scene.camera.pickEllipsoid` 方法存在一个根本性的局限：**它只会在椭球体（即WGS84这种没有地形起伏的“光滑球面”）表面进行交点计算**。这意味着，即使开启了地形（Terrain），该方法也会直接穿过山脉，只返回海平面（高度约为0）的坐标。
...
handler.setInputAction((e) => {
  ...
  if (Cesium.defined(pickPosition)) {
    ...
    const height = cartographic.height; // 高度可能始终为0
