...
> [!NOTE] 工作原理
>
> - **唯一标识（JSESSIONID）**：当客户端首次访问服务器时，服务器为该客户端创建一个 `HttpSession` 对象，并生成一个唯一的字符串 ID（即 JSESSIONID）；
...
HttpSession 用来创建一个或多个 HTTP 客户端之间的 Session 会话，会话可以持续指定的时间段，跨多个连接或页面请求。
...
| 方法                      | 描述                                                 |
| ------------------------- | ---------------------------------------------------- |
| getAttribute(name)        | 获取会话种指定名称的对象，未找到返回 null            |
