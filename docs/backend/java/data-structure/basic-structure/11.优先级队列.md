...
[堆](https://cwt113.github.io/geomind/backend/java/data-structure/basic-structure/8.%E5%A0%86) 是一种基于树得数据结构，通常用完全二叉树实现。堆又可以分为 大顶堆 和 小顶堆。

> [!NOTE] 优点
>
> - 相比于有序数组和无序数组的实现方式，**用堆平衡了入队和出队的时间复杂度**，入队和出队复杂度为 $O(log_n)$，`peek()` 复杂度为 $O(1)$；
> - 二叉堆只需要一个数组存储，不需要链表额外指针，因此 **结构紧凑、内存连续**，CPU 缓存命中率高，运行速度更快；

> [!TIP] 堆计算父节点和子节点
>
> - 当 $i > 0$ 时，节点 $i$ 的父节点为 $floor((i - 1) / 2)$；
> - 当 $0 < i < size$ 时，节点 $i$ 的左子节点为 $2i + 1$，右子节点为 $2i + 2$；

::: code-group

```java [PriorityQueue] {10,36,52,60,65,70}
public class PriorityQueue3<E extends Entry> implements Queue<E> {
  private final Priority[] array;
  private int size;
  ...
  public boolean offer(E value) {
    /**
		 * 入队逻辑：
		 * 1.入队新元素，先加入到数组末尾（索引位置记为 child）；
		 * 2.不断循环比较新加元素和父节点（parent）节点的优先级：
		 * 	2.1 如果父节点的优先级比子节点底，则父节点下潜至子节点 child 的位置；
		 * 	2.2 此时子节点上浮 child = parent，parent 继续找父节点；
		 * 	2.3 如果没有父节点了（child = 0），或者父节点优先级高于子节点了，结束循环；
		 */
    if (isFull()) {
      return false;
    }
  ...
  /**
	 * 节点下潜
	 * @param parent 下潜开始的父节点索引
	 */
  private void down(int parent) {
    int left = parent * 2 + 1;
    int right = left + 1;

    int max = parent; // 假设第一个父节点的优先级最高
