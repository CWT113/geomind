...
public class SinglyLinkedList implements Iterable<Integer> {
  ...
  public void recursion(Node curr, Consumer<Integer> before, Consumer<Integer> after) {
    ...
    before.accept(curr.value); // 正序遍历
    recursion(curr.next, before, after);
    after.accept(curr.value); // 倒序遍历（因为递归的因素）
  }
}
```
...
public class DoublyLinkedListSentinel implements Iterable<Integer> {
  ...
  public void insert(int index, int value) {
    Node prev = findNode(index - 1);
    if (prev == null) {
      throw new IndexOutOfBoundsException(String.format("index [%d] 不合法", index));
    }
    Node next = prev.next;
  ...
  public void removeLast() {
    ...
    // 链表为空时，尾节点的上一个元素是头节点时，抛异常
    if (removed == head) {
      throw new IndexOutOfBoundsException(String.format("index [%d] 不合法", 0));
    }
...
环形链表的最后一个节点指向第一个节点，形成闭环结构。而单向环形链表又是双向环形链表的一种特殊情况。

![image-20251109145050348](./assets/image-20251109145050348.png)
